...
public abstract class GenericAudioAssembler implements IAssembler {
	...
	public Frame createCompleteFrame() throws Exception {
		return this.createCompleteFrame(inputTools);
	}

	
	public Frame createCompleteFrame(InputTools io) throws Exception {
		Frame frame = null;
		
		try {		
			// Percorre string com os bytes do fluxo coletando frames
			if (io.lookBitsAhead(constants.SYNC_WORD)) {
				frame = this.createFrame(io);
	 ...
	 */	
	protected abstract Frame createFrame(InputTools io) throws Exception;
